...
# Map of security group ARNs to their respective repo_id tag values
...
# Use null_resource to apply only the repo_id tag using AWS CLI
resource "null_resource" "tag_security_groups" {
  for_each = var.security_group_tags

  provisioner "local-exec" {
    # Inline extraction of security group ID and application of the repo_id tag
    command = <<EOT
